...
import { NoAlgorithm } from "../../classes/display/no.algorithm";
...
export class DisplayComponent implements OnInit, OnDestroy {
    ...
    // Layout algorithm can be swapped out dynamically (NoAlgorithm or SpringEmbedderAlgorithm)
    private _layoutAlgorithm!: LayoutAlgorithmInterface;
    ...
    private viewBox: { x: number, y: number, w: number | undefined, h: number | undefined } = {
        x: 0,
        y: 0,
        w: undefined,
        h: undefined
    ...
    constructor(
        ...
        // Inject both algorithms; we will switch them at runtime
        private noAlgorithm: NoAlgorithm,
        private springEmbedderAlgorithm: SpringEmbedderAlgorithm,
        private _settings: SettingsSingleton,
        private _http: HttpClient
    ) {
        // Subscribe to DisplayService for graph updates
        this._sub.add(
            this._displayService.graph$.subscribe(graph => {
    ...
    ngOnInit(): void {
        // React to changes in SettingsSingleton (specifically springEmbedderDisabled)
        this._sub.add(
            this._settings.state$.subscribe(settings => {
                if (settings.springEmbedderDisabled) {
                    this._layoutAlgorithm = this.noAlgorithm;
                } else {
                    this._layoutAlgorithm = this.springEmbedderAlgorithm;
                }
                // Optionally force a re-draw whenever the setting changes
                this.draw();
            })
        );

        // Initialize layout algorithm (in case subscription above hasn't emitted yet)
    ...
    public processMouseWheel(inEvent: WheelEvent) {
        ...
        this.drawingArea?.nativeElement.setAttribute('viewBox',
    ...
    public endCut(inEvent: MouseEvent): void {
        ...
        if (this._graph?.arcs) {
            for (const arc of this._graph.arcs) {
                const existingLine = {
                    x1: arc.sourceX, y1: arc.sourceY,
                    x2: arc.targetX, y2: arc.targetY
                };
                for (const cutLine of this.activeCut.tempCutLines) {
                    // If already in the cutArcs array, skip
                    if (this.activeCut.cutArcs.some(cutArc =>
                        cutArc.source.id == arc.source.id && cutArc.target.id == arc.target.id)) {
                        continue;
                    }
    ...
    public resetZoom() {
        this.viewBox = {
            x: 0,
            y: 0,
            w: this.drawingArea?.nativeElement.clientWidth,
            h: this.drawingArea?.nativeElement.clientHeight
        };
        this.drawingArea?.nativeElement.setAttribute(
            'viewBox',
            `${this.viewBox.x} ${this.viewBox.y} ${this.viewBox.w} ${this.viewBox.h}`
        );
    }
        ...
        this._http.get(inLink, { responseType: 'text' })
            ...
            .subscribe(fileContent => {
                ...
                if (fileType !== undefined) {
                    ...
                    throw new Error('#cmp.dsp.ftf.000: filetype was "undefined"');
                }
            });
    }
    ...
    private readFile(files: FileList | undefined | null): void {
        ...
        if (fileType !== undefined) {
            ...
            throw new Error('#cmp.dsp.rdf.000: filetype was "undefined"');
        }
    }
    ...
    /**
     * Draw (render) the current Graph in the SVG.
     * Uses the currently-active layout algorithm (set by springEmbedderDisabled).
     */
    private draw(): void {
        if (!this.drawingArea) {
        ...
        // Dynamically apply the active layout algorithm:
        this._graph = this._layoutAlgorithm.applyLayout(this._graph);

        this.clearDrawingArea();

        // Create static SVG layers (arcs, nodes, etc.)
        const svgLayers: [SVGElement[], SVGElement[], SVGElement[]] =
            this._svgService.createSvgStatics(this._graph);

        const [svgGraphElements, svgDescriptors, svgInfoBoxes] = svgLayers;
        ...
        // (Optional) Only create traces if not dragging:
        if (!this.dragInProgress) {
